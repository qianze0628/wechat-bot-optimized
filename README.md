...
- **npm 安装跨平台无差异**: 本项目依赖 (wechat4u / axios / lodash 等) 全部为纯 JavaScript,无原生编译模块,Windows / Linux / macOS 上 `npm install` 结果一致。
- **登录稳定性受微信风控影响,与操作系统无关**: 本项目使用微信**旧网页版协议**(wechat4u),微信官方对网页版登录有风控(检测 UA、环境特征、登录频率等)。Windows 桌面环境特征更接近普通用户,而 Linux 服务器 / Docker 容器环境特征更"非典型",**更容易被微信判定为异常登录而掉线**——这是协议层面的限制,不是代码问题。
- **建议**: 个人使用优先 Windows;Linux/macOS 上如遇掉线,可适当降低登录/消息频率,或考虑商业协议方案(如 iPad 协议, 见下方说明)。

## 免责声明

本项目仅供学习与技术交流。请遵守微信使用规范,控制频率,勿用于骚扰、营销等用途。使用非官方微信协议存在账号风险,请自行评估并承担后果。
